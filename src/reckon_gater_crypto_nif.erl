...
%% pure Erlang fallbacks in their respective modules (reckon_gater_identity, etc.).
%%
...
-module(reckon_gater_crypto_nif).
...
-define(NIF_LOADED_KEY, reckon_gater_crypto_nif_loaded).
...
-spec init() -> ok.
init() ->
    NifName = "reckon_gater_crypto_nif",
    Paths = nif_search_paths(NifName),
    case try_load_nif(Paths) of
        ok ->
            persistent_term:put(?NIF_LOADED_KEY, true),
            logger:info("[reckon_gater_crypto_nif] NIF loaded - Enterprise mode"),
            ok;
        {error, Reason} ->
            persistent_term:put(?NIF_LOADED_KEY, false),
            logger:info("[reckon_gater_crypto_nif] NIF not available (~p), using pure Erlang - Community mode",
                       [Reason]),
            ok
    end.
...
%% Falls back to reckon_gater_identity:base58_encode/1 if NIF unavailable.
%%
...
%% Falls back to reckon_gater_identity:base58_decode/1 if NIF unavailable.
%%
