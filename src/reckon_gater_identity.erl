...
%%   {ok, PubKey} = reckon_gater_identity:public_key_from_did(DID).
%%
%% @author rgfaber

-module(reckon_gater_identity).
...
is_nif_available() ->
    try
        reckon_gater_crypto_nif:is_loaded()
    catch
        _:_ -> false
    end.
...
base58_encode(Bin) ->
    case is_nif_available() of
        true ->
            reckon_gater_crypto_nif:base58_encode(Bin);
        false ->
...
base58_decode(Base58) ->
    case is_nif_available() of
        true ->
            reckon_gater_crypto_nif:base58_decode(Base58);
        false ->
