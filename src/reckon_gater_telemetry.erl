...
%%   ok = reckon_gater_telemetry:attach(my_handler, fun my_module:handle/4, #{}).
%%
%% Emit an event:
%%   reckon_gater_telemetry:emit(?GATER_REQUEST_STOP, #{duration => 1000}, #{store_id => my_store}).
%%
%% @author rgfaber

-module(reckon_gater_telemetry).
...
-define(HANDLER_ID, reckon_gater_telemetry_handler).

%%====================================================================
%% API
%%====================================================================

%% @doc Attach the default logger handler for all reckon_gater events
...
%% @doc Attach a custom handler for all reckon_gater events
...
%% Channel events
handle_event([reckon_gater, channel, broadcast], #{recipient_count := Count}, Meta, _Config) ->
...
%% @private Get all reckon_gater telemetry events
