%% @doc EUnit tests for reckon_gater_telemetry module
%% @author rgfaber

-module(reckon_gater_telemetry_tests).
...
cleanup(_) ->
    catch reckon_gater_telemetry:detach_default_handler(),
    ok.
...
attach_detach_default_handler_test() ->
    ...
    Result1 = reckon_gater_telemetry:attach_default_handler(),
    ?assertEqual(ok, Result1),

    %% Should fail when already attached
    Result2 = reckon_gater_telemetry:attach_default_handler(),
    ?assertEqual({error, already_exists}, Result2),

    %% Should be able to detach
    Result3 = reckon_gater_telemetry:detach_default_handler(),
    ?assertEqual(ok, Result3),

    %% Should be able to attach again
    Result4 = reckon_gater_telemetry:attach_default_handler(),
    ?assertEqual(ok, Result4),

    %% Cleanup
    reckon_gater_telemetry:detach_default_handler().

attach_detach_custom_handler_test() ->
    Handler = fun(_Event, _Measurements, _Meta, _Config) -> ok end,

    %% Should be able to attach
    Result1 = reckon_gater_telemetry:attach(test_handler, Handler, #{}),
    ?assertEqual(ok, Result1),

    %% Should fail when already attached
    Result2 = reckon_gater_telemetry:attach(test_handler, Handler, #{}),
    ?assertEqual({error, already_exists}, Result2),

    %% Should be able to detach
    Result3 = reckon_gater_telemetry:detach(test_handler),
    ?assertEqual(ok, Result3).
...
emit_worker_registry_events_test() ->
    ...
    ok = reckon_gater_telemetry:emit(
...
emit_channel_broadcast_events_test() ->
    ...
    ok = reckon_gater_telemetry:emit(
        ...
        #{recipient_count => 5},
        #{channel => reckon_gater_channel_events, topic => <<"events.test">>}
    ),
...
custom_handler_receives_events_test() ->
    ...
    ok = reckon_gater_telemetry:attach(custom_test_handler, Handler, #{}),
    ...
    reckon_gater_telemetry:detach(custom_test_handler).
