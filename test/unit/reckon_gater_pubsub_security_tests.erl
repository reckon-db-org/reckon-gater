%% @doc EUnit tests for reckon_gater_pubsub_security module
%% @author rgfaber

-module(reckon_gater_pubsub_security_tests).
...
setup() ->
    ...
    reckon_gater_pubsub_security:set_secret(<<"test_secret_key_12345">>),
    ok.
...
verify_valid_signature_test() ->
    ...
    ?assertEqual(ok, reckon_gater_pubsub_security:verify(SignedMessage)).
...
verify_invalid_signature_test() ->
    ...
    ?assertEqual({error, invalid_signature}, reckon_gater_pubsub_security:verify(TamperedMessage)).

verify_missing_signature_test() ->
    Message = #{type => test, data => <<"hello">>},

    ?assertEqual({error, missing_signature}, reckon_gater_pubsub_security:verify(Message)).
...
verify_expired_message_test() ->
    ...
    Secret = reckon_gater_pubsub_security:get_secret(),
    ...
    ?assertEqual({error, message_expired}, reckon_gater_pubsub_security:verify(SignedMessage)).

sign_custom_secret_test() ->
    Message = #{type => test, data => <<"hello">>},
    CustomSecret = <<"my_custom_secret">>,

    SignedMessage = reckon_gater_pubsub_security:sign(Message, CustomSecret),

    %% Should verify with same secret
    ?assertEqual(ok, reckon_gater_pubsub_security:verify(SignedMessage, CustomSecret)),

    %% Should fail with different secret
    ?assertEqual({error, invalid_signature},
                 reckon_gater_pubsub_security:verify(SignedMessage, <<"wrong_secret">>)).

set_get_secret_test() ->
    %% Set a new secret
    NewSecret = <<"new_test_secret">>,
    ok = reckon_gater_pubsub_security:set_secret(NewSecret),

    %% Get it back
    ?assertEqual(NewSecret, reckon_gater_pubsub_security:get_secret()),
...
secure_compare_test() ->
    ...
    Message1 = #{data => <<"test">>},
    Signed1 = reckon_gater_pubsub_security:sign(Message1),
    ?assertEqual(ok, reckon_gater_pubsub_security:verify(Signed1)),

    %% Different length signatures should fail
    TamperedShort = Signed1#{signature => <<"short">>},
    ?assertEqual({error, invalid_signature}, reckon_gater_pubsub_security:verify(TamperedShort)).
