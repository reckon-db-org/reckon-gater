%% @doc EUnit tests for reckon_gater_retry module
%% @author rgfaber

-module(reckon_gater_retry_tests).
...
calculate_delay_exponential_test() ->
    ...
    Delay0 = reckon_gater_retry:calculate_delay(0, Config),
    ?assert(Delay0 >= 100),
    ?assert(Delay0 =< 125),  %% max jitter is 25%

    %% Attempt 1: 100 * 2^1 = 200 (+ jitter)
    Delay1 = reckon_gater_retry:calculate_delay(1, Config),
    ?assert(Delay1 >= 200),
    ?assert(Delay1 =< 250),

    %% Attempt 2: 100 * 2^2 = 400 (+ jitter)
    Delay2 = reckon_gater_retry:calculate_delay(2, Config),
    ?assert(Delay2 >= 400),
    ?assert(Delay2 =< 500).
...
calculate_delay_max_test() ->
    ...
    Delay = reckon_gater_retry:calculate_delay(10, Config),
...
default_config_test() ->
    Config = reckon_gater_retry:default_config(),
