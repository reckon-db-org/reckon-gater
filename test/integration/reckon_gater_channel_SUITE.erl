%% @doc Common Test suite for reckon_gater channel system
%%
...
-module(reckon_gater_channel_SUITE).
...
init_per_suite(Config) ->
    ...
    %% Set HMAC secret
    reckon_gater_pubsub_security:set_secret(<<"test_secret_for_suite">>),
    Config.
...
channel_starts(Config) ->
    ...
    {ok, Pid} = reckon_gater_channel_server:start_link(reckon_gater_channel_events, #{name => test_events_channel}),
    ?assert(is_pid(Pid)),
...
channel_priority(_Config) ->
    ...
    ?assertEqual(high, reckon_gater_channel_events:priority()),
    ?assertEqual(critical, reckon_gater_channel_alerts:priority()),
    ?assertEqual(normal, reckon_gater_channel_system:priority()),
    ?assertEqual(low, reckon_gater_channel_logging:priority()),
    ok.
...
subscribe_receive_message(Config) ->
    %% Start a test channel
    {ok, Pid} = reckon_gater_channel_server:start_link(reckon_gater_channel_events, #{name => test_pubsub_channel}),

    Topic = <<"test.topic.1">>,

    %% Subscribe
    ok = reckon_gater_channel_server:subscribe(test_pubsub_channel, Topic, self()),

    %% Publish a message
    Message = #{type => test_event, data => <<"hello">>},
    ok = reckon_gater_channel_server:publish(test_pubsub_channel, Topic, Message),
...
unsubscribe_stops_messages(Config) ->
    {ok, Pid} = reckon_gater_channel_server:start_link(reckon_gater_channel_events, #{name => test_unsub_channel}),

    Topic = <<"test.topic.2">>,

    %% Subscribe
    ok = reckon_gater_channel_server:subscribe(test_unsub_channel, Topic, self()),

    %% Unsubscribe
    ok = reckon_gater_channel_server:unsubscribe(test_unsub_channel, Topic, self()),
    ...
    ok = reckon_gater_channel_server:publish(test_unsub_channel, Topic, Message),
...
multiple_subscribers(Config) ->
    {ok, Pid} = reckon_gater_channel_server:start_link(reckon_gater_channel_events, #{name => test_multi_channel}),
    ...
    %% Subscribe both
    ok = reckon_gater_channel_server:subscribe(test_multi_channel, Topic, Sub1),
    ok = reckon_gater_channel_server:subscribe(test_multi_channel, Topic, Sub2),

    %% Publish
    Message = #{type => broadcast, data => <<"to all">>},
    ok = reckon_gater_channel_server:publish(test_multi_channel, Topic, Message),
...
rate_limiting_applies(Config) ->
    ...
    {ok, Pid} = reckon_gater_channel_server:start_link(reckon_gater_channel_health, #{name => test_rate_channel}),

    Topic = <<"health.test">>,

    %% Send many messages quickly
    Results = [reckon_gater_channel_server:publish(test_rate_channel, Topic, #{i => I}) || I <- lists:seq(1, 150)],
...
hmac_required_channel(Config) ->
    ...
    {ok, Pid} = reckon_gater_channel_server:start_link(reckon_gater_channel_alerts, #{name => test_hmac_channel}),

    Topic = <<"alerts.test">>,

    %% Publish a properly signed message
    Message = #{type => alert, level => critical, message => <<"Test alert">>},
    SignedMessage = reckon_gater_pubsub_security:sign(Message),

    Result = reckon_gater_channel_server:publish(test_hmac_channel, Topic, SignedMessage),
...
hmac_verification_fails_invalid(Config) ->
    {ok, Pid} = reckon_gater_channel_server:start_link(reckon_gater_channel_alerts, #{name => test_hmac_fail_channel}),

    Topic = <<"alerts.test">>,

    %% Try to publish unsigned message
    UnsignedMessage = #{type => alert, message => <<"Unsigned">>},
    Result1 = reckon_gater_channel_server:publish(test_hmac_fail_channel, Topic, UnsignedMessage),
    ...
    Result2 = reckon_gater_channel_server:publish(test_hmac_fail_channel, Topic, InvalidMessage),
